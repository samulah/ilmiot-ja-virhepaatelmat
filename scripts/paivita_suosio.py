...
# "Eniten kasvua" -lista käyttää samaa tasoitettua suhdetta mutta matalampaa
...
def kasvulista(nyt_summat, ennen_summat, tunnetut, viikko, lattia,
    ...
    Kolme ehtoa, ja jokainen on siellä siksi ettei lista valehtelisi:
      ...
      * lattia karsii sivut joilla suhde on kohinaa (2 → 5 on +150 %),
      * `nyt > ennen` vaaditaan ehdottomana: pelkkä tasoitettu suhde nostaisi
        listalle myös paikallaan pysyneitä, koska tasoitus vetää kaikki kohti
...
/* Kolmen listan rivi mahtuu vierekkäin kapeammalla minimillä; nimet katkeavat
   ellipsiin, joten kapea sarake ei riko taittoa. */
.nostot-kolme { grid-template-columns: repeat(auto-fit, minmax(270px, 1fr)); }
.nosto {
  background: #FDFAF2;
...
/* Kasvuprosentti näkyy myös tuotannossa: se ei paljasta liikennemäärää,
   ja ilman lukua rivi on pelkkä nimi ilman perustetta järjestykselle. */
#kasvu-lista .nosto-rivi-luku { color: #1e7a4a; font-weight: 600; }
...
<section class="nostot" id="nostot" hidden>
  ...
  <div class="nostot-parit nostot-kolme">
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function lukuMuotoilu(r) {
    return NAYTA_LUVUT ? r.n.toLocaleString('fi-FI') : '';
  }

  function rakennaLista(tunnus, oletusIkkuna, saate, muotoile, minRivit) {
    ...
    const min = minRivit || 1;

    // Rivit jotka oikeasti piirtyisivät. Data ei riitä: sivun on myös oltava
    // olemassa korttilistassa, jottei poistettu ilmiö pidä välilehteä
    // näkyvissä tyhjänä.
    function kelvolliset(ikkuna) {
      return (data[ikkuna] || [])
        .filter(function (r) { return kortit.has(r.u); }).slice(0, 6);
    }

    // Jokainen välilehti arvioidaan erikseen samalla kynnyksellä kuin lohko:
    // tyhjä tai liian lyhyt "30 pv" olisi lupaus jota klikkaus ei lunasta.
    // Näin 30 pv:n kasvu ilmestyy itsestään sinä päivänä kun kannassa on kaksi
    // täyttä 30 pv:n ikkunaa, eikä väliaikaa varten tarvita erillistä lippua.
    //
    // Kynnys koski aiemmin vain oletusikkunaa ja välilehti piilotettiin vasta
    // täysin tyhjänä. Kahden rivin "7 pv" putosi silloin väliin: ei tarpeeksi
    // tyhjä vaihtamaan oletusta, ei tarpeeksi täysi läpäisemään minimiä — ja
    // vieressä ollut kuuden rivin "30 pv" jäi näkymättä kokonaan (16.8.2026).
    const kaikki = Array.from(
      document.querySelectorAll('.nosto-valilehti[data-lista="' + tunnus + '"]'));
    kaikki.forEach(function (n) {
      n.hidden = kelvolliset(n.dataset.ikkuna).length < min;
    });
    ...
    // Oletusikkuna kelpaa vain jos se itse yltää kynnykseen; muuten ensimmäinen
    // joka yltää. Jos yksikään ei yllä, lohkoa ei ole mitään syytä näyttää.
    if (kelvolliset(oletusIkkuna).length < min) {
      if (!valilehdet.length) return false;
      oletusIkkuna = valilehdet[0].dataset.ikkuna;
    }

    function piirra(ikkuna) {
      const rivit = kelvolliset(ikkuna);
      lista.textContent = '';
  ...
  if (tuore) {
    ...
    rakennaLista('kasvu', 'k7',
      ...
      return '+' + p + ' %' + (NAYTA_LUVUT ? ' · ' + r.edellinen + '→' + r.n : '');
    }, 3);
  }
...
<div class="parit">
  <section>
    <h2>Nousijat</h2>
    <p class="selite">7 pv vs. edellinen 7 pv, lattian {VIIKKO_LATTIA} ylittäneet.
    Etusivun &quot;Eniten kasvua&quot; -lohko käyttää matalampaa lattiaa
    ({KASVU_LATTIA}) ja jättää viikon ilmiön pois, joten listat eivät ole samat.</p>
    {nousu_taulu}
  </section>
  <section>
    <h2>Laskijat</h2>
